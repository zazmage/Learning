        ...
        while True:
            try:
                ...
                self.active_queue.put(
                    ((sock, addr), self.interface[1], self.secure)
                )
...
class ThreadPool:
    ...
    def __init__(
        ...
        if has_futures and app_info.get("futures"):
            app_info["executor"] = WSGIExecutor(
                max([DEFAULTS["MIN_THREADS"], 2])
            )
    ...
    def _handleError(self, typ, val, tb):
        ...
        if typ == socket.error:
            ...
            if val.args[0] in IGNORE_ERRORS_ON_CLOSE:
                if __debug__:
                    self.err_log.debug(
                        "Ignorable socket Error received..."
                        "closing connection."
                    )
                return False
            else:
...
def demo_app(environ, start_response):
    ...
    if static_folder:
        ...
        data = (
            "<html><body><h1>Hello from Rocket Web Server</h1></body></html>"
        )
    return [data]
